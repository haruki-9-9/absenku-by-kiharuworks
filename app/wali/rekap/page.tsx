...
export default function WaliRekapPage() {
  ...
  const handleLihat = () => {
    if (jenis === "bulanan") {
      router.push(`/wali/rekap/bulanan?bulan=${bulan}&tahun=${tahun}`);
    } else {
      if (!semesterId) return;
      router.push(`/wali/rekap/semester?semesterId=${semesterId}`);
    }
  };
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 24, maxWidth: 520 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "#64748b", marginTop: 2 }}>
            Pilih periode untuk melihat rekap kelas kamu.
          </p>
        </div>
                ...
                <div style={{ position: "relative" }}>
                  <select value={semesterId} onChange={(e) => setSemesterId(e.target.value)} style={selectStyle}>
                    {tahunAjaranList.map((ta) =>
                      ta.semester.map((sem) => (
                        <option key={sem.id} value={sem.id}>{sem.nama} — {ta.nama}</option>
                      ))
                    )}
                  </select>
                  <svg style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", pointerEvents: "none" }}
